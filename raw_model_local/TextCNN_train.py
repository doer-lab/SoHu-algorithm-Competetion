...
tf.flags.DEFINE_integer("num_labels", 3, "Number of labels for data")
...
x = np.array(data_helper.sentence2vector(train_data.news_words_jieba,length_per_sequence=sequence_length,word2vec_model_path=FLAGS.word2vec_model_path))
y = np.array(train_label.news_pic_label_one_hot)

# release memmory: delete some nouseful data
del train_data, train_label, validate_data
...
with tf.Graph().as_default():
    ...
    with tf.Session(config = session_conf) as sess:
        ...
        learning_rate = tf.train.exponential_decay(FLAGS.learning_rate_basic,
                                           global_step, x_train.shape[0] / FLAGS.batch_size,
