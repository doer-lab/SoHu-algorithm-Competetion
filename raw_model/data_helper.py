...
def sentence2vector(sentence_need_to_vector,length_per_sequence,word2vec_model_path=None):
    ...
    print("Padding is finished!\nNumber of examples is {}, the length of per sentence after padding is {}".format(len(sentences),padding_sentence_length))
    
    print("Word representation is starting ...")
    ...
    print('Embedding dimension is ', embeddingDim)          # 嵌入维数
    ...
    print("Embedding is fnished (or sentences to vectors is over)!".center(100,'#'))
    return all_vectors

def batch_iter(data, batch_size, num_epochs, shuffle=True):
    '''
    Generate a batch iterator for a dataset
    Total num_epochs * num_batches_per_epoch iters
    '''
    data = np.array(data)
...
def batch_concat(need_concatenate_list,batch_size=300,num_iters=None):
    ...
    for iter in range(num_iters):
        ...
        end_idx = min((iter + 1)*batch_size, len(need_concatenate_list))
        yield np.array(need_concatenate_list[start_idx: end_idx])
